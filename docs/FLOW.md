...
5. [x] **Checkout and enrollment** (`#5`)
   - [x] Build wallet top-up checkout page. (checkout API now creates a pending wallet transaction + hosted checkout URL)
   - [x] Integrate payment provider callback/webhook verification for wallet top-up.
   - [x] Create order/enrollment only when user buys course from wallet balance.
   - [x] Prevent duplicate purchase for already enrolled users.
   - [x] Handle failed/cancelled/refunded top-up transactions.
